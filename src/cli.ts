  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--port' || a === '-p') {
      ...
      if (!Number.isInteger(n) || n < 0 || n > 65535) {
        console.error(`archiviz: invalid --port value "${raw}" (must be integer 0-65535).`);
        process.exit(1);
      }
      port = n;
    } else if (a === '--no-open') {
...
export function printHelp(): void {
  const v = getVersion();
  console.log(`archiviz v${v} - local-first code architecture explorer
...
EXAMPLES
  archiviz
  archiviz .
  archiviz /path/to/project --port 5000 --no-open
  archiviz ~/code/my-app --include-hidden
...
export function printVersion(): void {
  console.log(`archiviz v${getVersion()}`);
}
